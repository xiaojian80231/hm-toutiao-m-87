    ...
    <div class="channel">
      ...
      <van-grid class="van-hairline--left">
        <van-grid-item v-for="(item,i) in myChannels" :key="item.id">
          <span @click="enterChannel(i)" class="f12" :class="{blue:activeIndex===i}">{{item.name}}</span>
...
export default {
  ...
  data () {
    return {
      editing: false, // 控制 编辑状态（默认为false不编辑）
      allChannels: [] // 全部频道
    }
  },
  computed: {
    ...
    optionalChannels () {
      // 可选频道（计算属性） = 全部频道 - 我的频道
      // 减法逻辑：
      // 1：先遍历全部频道，拿着当前遍历的频道ID，去我的频道中查找有没有相同ID的频道
      // 2：查到相同的频道ID 排除不要
      // 3：查询不到相同的频道ID 这就是可选频道  追加到新数组中即可
  ...
  methods: {
    ...
    async delChannel (index, channelId) {
      ...
      // 不能修改(简单数据类型：不能修改) (复杂数据类型：不能修改引用地址，可以修改值，不能重新赋值)
