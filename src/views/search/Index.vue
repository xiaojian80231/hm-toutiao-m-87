...
// 1.3：组件初始化 获取数据  模板中使用即可
// 2：删除历史
// 2.1：绑定name="cross" 点击事件  传入索引
// 2.2：根据索引经行删除即可，同时要更新本地存储
// 3：点击历史文字  进入搜索结果
...
export default {
  ...
  data () {
    return {
      q: '', // 搜索关键字
      historyList: JSON.parse(window.localStorage.getItem(KEY) || '[]'), // 历史搜索列表
      options: [], // 词条
      timer: null// 计数器
    }
  },
  watch: {
    q () {
      ...
      // 输入内容改变的时候执行该函数
      // 需求：当用户停止输入后500ms后发送请求，如果500ms内再次输入内容，重新计时
      // 函数防抖
      window.clearTimeout(this.timer)
  ...
  methods: {
    onSearch (key) {
      // PC端按下回车键触发，此为M端-点击虚拟键盘中的  enter键|搜索键  触发
      // key 是当前输入的文字
      if (!key) return false
      // 1：存储本地
